            ...
            try:
                ...
                for idx, img in enumerate(new_images):
                    with st.spinner(f"Processing Image {img.name} ({idx+1}/{len(new_images)})..."):
                        ...
                        try:
                            cnn_result = run_pickle_model_prediction(img_file_path)

                            if cnn_result:
                                st.success(f"🧠 CNN Prediction: {cnn_result['prediction']}")
                                st.info(f"Confidence: {cnn_result['confidence']*100:.2f}%")
                            else:
                                st.error(f"Some error happened: {e}")
                            
                            findings_text = f"""
                                            CNN MODEL PREDICTION:
                                            Predicted Class: {cnn_result['prediction']}
                                            Confidence: {cnn_result['confidence']*100:.2f}%
                                            """
